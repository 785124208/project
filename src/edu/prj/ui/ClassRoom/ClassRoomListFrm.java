...
public class ClassRoomListFrm extends JFrame implements java.io.Serializable{
...
//		String searchBook=txtSearchBook.getText();
		if(searchName!=null&&!searchName.isEmpty()) {
			list=classRoomService.listByName(searchName);
		}else {
		...
		tableModel = new ATableModel<ClassRoom>(list, 3) {
			...
			@Override
			public Object getPropValue(ClassRoom bean, int columIndex) {
				// TODO Auto-generated method stub

				ClassRoom item = (ClassRoom) bean;
				if (columIndex == 0) {
					return item.getRoomId();
				} else if (columIndex == 1) {
					return item.getRoomName();
				} else if (columIndex == 2) {
					return item.getGradeId();
				}
				return null;
			}
	...
	public void btnInsert_click(ActionEvent e) {
		...
		classRoomInsertFrm.classRoomListFrm=this;
		this.dispose();
	...
	private void updateMenuItem_click(ActionEvent e) {
		...
		if(index!=-1) {
			...
			if(pk>0) {
				...
				classRoomUpdateFrm.classRoomListFrm=this;
				this.dispose();
