...
public class TeacherListFrm extends JFrame implements java.io.Serializable{
...
//		String searchBook=txtSearchBook.getText();
		if(searchName!=null&&!searchName.isEmpty()) {
			list=teacherService.listByName(searchName);
		}else {
			list=teacherService.list();
		}
		ATableModel<Teacher> tableModel = null;
		tableModel = new ATableModel<Teacher>(list, 5) {

			@Override
			public Object getPropValue(Teacher bean, int columIndex) {
				// TODO Auto-generated method stub

				Teacher item = (Teacher) bean;
				if (columIndex == 0) {
	...
	public void btnInsert_click(ActionEvent e) {
		...
		teacherInsertFrm.teacherListFrm=this;
		this.dispose();
	...
	private void updateMenuItem_click(ActionEvent e) {
		...
		if(index!=-1) {
			...
			Long pk=Long.valueOf(""+obj);
			if(pk>0) {
				TeacherUpdateFrm teachUpdateFrm=new TeacherUpdateFrm();
				teachUpdateFrm.pk=pk;
				teachUpdateFrm.loadData();
				teachUpdateFrm.teacherListFrm=this;
				this.dispose();
				teachUpdateFrm.setVisible(true);
			}
			}
	}
}
